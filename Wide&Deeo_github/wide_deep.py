...
wide部分需要将连续特征离散化，有些时候连续特征和label之间不是线性的关系。可能刚开始是正的线性关系，后面又变成了负的线性关系。
现在Wide部分用了一个离散化的连续特征age_buckets，其他的连续特征没有使用
"""
...
model = tf.estimator.DNNLinearCombinedClassifier(
    model_dir=model_dir,
    linear_feature_columns=base_columns + crossed_columns,  # 线性模型权重更新的 optimizer
    dnn_feature_columns=deep_columns,  # 输入 DNN 模型的 feature columns
    dnn_hidden_units=[100, 50]  # DNN模型的隐藏层神经元数目
)
...
def input_fn(data_file, num_epochs, shuffle, batch_size):
    ...
    # todo:需要确定shuffle/repeat/batch的意义（数据集足够是否不需要repeat）
